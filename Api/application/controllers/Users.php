...
class Users extends REST_Controller
{
    ...
    public function collectors_list_get()
    {
	  ...
	  //Get Collectors List
    public function block_rows_post()
    {   
        $data = file_get_contents('php://input');

        $collector = json_decode($data);
        $collector_tag = trim($collector->collector_id);

        $block_rows = $this->users_model->all_block_row($collector_tag);
    ...
    public function collector_unit_time_post()
    {
        ...
        $last_sync = !empty($buck->row()->last_sync) ? date('d-m-Y H:i', strtotime($buck->row()->last_sync)) : '00-00-0000 00:00';
        ...
        if ($result == 1) {
            ...
            $message = array('status' => 9, 'msg' => 'Job is paused, Collector needs to resume the job..', 'last_sync' => $last_sync, 'data' => $data);
            $this->response($message, 200);
    ...
    public function collector_row_time_post()
    {
        ...
        if ($result == 1) {
            ...
            $message = array('status' => 9, 'msg' => 'Job is paused, Collector needs to resume the job..', 'data' => $data);
            $this->response($message, 200);
        } else {
            $message = array('status' => 0, 'msg' => 'Failed..');
            $this->response($message, 300);
    ...
    public function worker_time_job_per_row_post()
    {
        ...
        $this->db->where('row_details_worker.job_number', $job_number);
        $this->db->where('row_details_worker.worker_id', $worker);

        if($result == 3 || $result == 5){
			$this->db->where('row_details_worker.status', 'finished');
        }
        else if($result == 6){
        	$this->db->where('row_details_worker.status', 'paused');
        }
        else{
        	$this->db->where('row_details_worker.status', 'started');	
        }
    ...
    public function row_scan_time_post()
    {
        ...
        if ($result != 2) {
            ...
            $this->db->select('row_details_worker.* , worker.worker_name, sum(row_details_worker.units_picked) as units_picked');
            $this->db->from('row_details_worker');
            $this->db->join('worker', 'worker.nfc_tag_id = row_details_worker.worker_id');
            $this->db->where('job_number', $job_number);
        ...
        if ($result == 1) {
            ...
            $message = array('status' => 5, 'msg' => 'Row started successfully', 'data' => $data);
            $this->response($message, 200);
    ...
    public function bucket_scan_time_post()
    {
        ...
        $time = trim($details->scan_time);
        $deviceId = trim($details->device_id);
        $flag = trim($details->flag);

        $result = $this->users_model->bucket_time($job_number, $worker, $collector, $bucket, $time, $deviceId, $flag);
        ...
        if ($result == 1) {
            ...
            $message = array('status' => 5, 'msg' => 'Unit scanned!', 'data' => $data);
            $this->response($message, 200);
            ...
            $message = array('status' => 8, 'msg' => 'Worker lock out time not over..', 'data' => $data);
            $this->response($message, 200);
    ...
    public function pause_job_row_post()
    {
        ...
        if ($result == 1) {
            ...
            $message = array('status' => 4, 'msg' => 'Job resume!', "lastHitTime" => $time);
            $this->response($message, 200);
    ...
    public function unallocate_bucket_post()
    {
        ...
        $result = $this->users_model->unallocate_bucket_scan($job_number, $bucket, $worker);
    ...
    public function sync_offline_row_details_collector_post()
    {
        ...
        if(!empty($row_collectors)){
            $jobStatus = $row_collectors[0]->jobStatus;
            if($jobStatus == 'active' || $jobStatus == 'paused'){
                $jobNumber = $row_collectors[0]->job_number;
                $this->db->where('job_number', $jobNumber);
                $this->db->update('jobs', array('job_status' => $jobStatus));
            }
        }
    ...
    public function sync_offline_row_scan_post()
    {
        ...
        $last_sync = !empty($res['last_sync']) ? date('d-m-Y H:i', strtotime($res['last_sync'])) : '00-00-0000 00:00';

        if ($res) {
            $message = array('status' => 1, 'msg' => 'Data sync completed!', 'last_sync' => $last_sync, 'data' => $res['result']);
            $this->response($message, 200);
...
// ---------- Hit these api when Sync Offline is started [End]---------------------//
     ...
     public function abc_get(){
        $job='LV1806-118';
        $work='1e0fc77a';
        $this->users_model->updatePickedBucked($job, $work);
    }
    ...
    public function get_job_worker_post()
    {
        $data = file_get_contents('php://input');
        $details = json_decode($data);
        ...
        $result = $this->users_model->get_job_worker($job_number);

        if ($result) {
            $message = array('status' => 1, 'msg' => 'Worker list', 'data' => $result);
            $this->response($message, 200);
    ...
    public function sync_offline_picking_details_worker2_post()
    {
        $data = file_get_contents('php://input');
        $details = json_decode($data);
        $picking_workers = $details->data;
        $response = $this->users_model->sync_offline_worker_picking2($picking_workers);
    ...
    //Sync offline data of row_details_worker
    public function sync_offline_row_details_worker2_post()
    {
        $data = file_get_contents('php://input');
        $details = json_decode($data);
        $row_workers = $details->data;
        $response = $this->users_model->sync_offline_worker_row2($row_workers);
    ...
    //Worker time for job_per_bucket
    public function worker_time_per_bucket_for_all_post()
    {
        $data = file_get_contents('php://input');
        $details = json_decode($data);
        ...
        if(!empty($details->data)){
            foreach ($details->data as $key => $value){
                $job_number = trim($value->job_number);
                $worker = trim($value->worker);
                $type = trim($value->type);
                $time = trim($value->scan_time);
            ...
            $finalData = array('status' => 0, 'msg' => 'Failed..');
            $this->response($finalData, 200);
        }
        $finalData = array('status' => 1, 'msg' => 'Successfully', 'data' => $message);
        $this->response($finalData, 200);
    }
    ...
    public function worker_time_job_per_row_for_all_post()
    {
        $data = file_get_contents('php://input');
        $details = json_decode($data);
        if(!empty($details->data)){
